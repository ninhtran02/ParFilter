...
u_n_options <- list(c(2,3),c(2,4),c(3,4),c(3,5),c(4,5))
paral_options <- 1:50
  ...
  for(mu in mu_options){
    for(u_n in u_n_options){
      for(paral in paral_options){
        u <- u_n[1]
        n <- u_n[2]
        file.name <- paste("Independence/cpch_uleqn/","mu",mu,"setup",u,n,"pi1pi1",pi1pi1,"paral",paral,".RD",sep = "")
        #load(file = file.name)
        if (!file.exists(file.name)) {
          message("Missing: ", file.name)
          un <- as.integer(paste0(u_n, collapse = ""))
          failed_ref <-  rbind(failed_ref,c(pi1pi1,mu,un))
        }
      }
    }
  }
...
levels(FDR_dat$pi1pi1) <- c("Least-informative~(delta == 0.50)",
                            "Mildly~informative~(delta == 0.70)",
                            "Most~informative~(delta == 0.90)")
FDR_dat$un <- as.factor(x = FDR_dat$un)
levels(FDR_dat$un) <- c("2/[3]","2/[4]","3/[4]","3/[5]","4/[5]")
levels(FDR_dat$un) <- c("2 / '['*3*']'",
                        "2 / '['*4*']'",
                        "3 / '['*4*']'",
                        "3 / '['*5*']'",
                        "4 / '['*5*']'")
...
levels(TPR_dat$pi1pi1) <- c("Least-informative~(delta == 0.50)",
                            "Mildly~informative~(delta == 0.70)",
                            "Most~informative~(delta == 0.90)")
TPR_dat$un <- as.factor(x = TPR_dat$un)
levels(TPR_dat$un) <- c("2/[3]","2/[4]","3/[4]","3/[5]","4/[5]")
levels(TPR_dat$un) <- c("2 / '['*3*']'",
                        "2 / '['*4*']'",
                        "3 / '['*4*']'",
                        "3 / '['*5*']'",
                        "4 / '['*5*']'")
...
FDR_plot <- ggplot(data=FDR_dat, aes(x=mu,
                                     y = FDR,
                                     group = Methods,
                                     linetype=Methods,
                                     shape=Methods,
...
TPR_plot <- ggplot(data=TPR_dat, aes(x=mu,
                                     y = TPR,
                                     group = Methods,
                                     linetype=Methods,
                                     shape=Methods,
...
#pdf(file = "u_equal_n_Results_uleqn2.pdf",
#    width = 9.6, height = 27.2)
...
pdf(file = "u_less_n_Results_cpch_uleqn2.pdf",
