...
generate_covariates <- function(m,n){
  base_x <- rnorm(n = m, mean = 0, sd = 1)
  X <- c()
  sfactor <- rep(1,n)#(1:n)
  #sfactor <- (1:n)
  
  for(j in 1:n){
    #X <- cbind(X, sfactor[j]*( base_x + 0.2*sin(10*base_x) ))
    #X <- cbind(X, sfactor[j]*( base_x + base_x^3 ))
    X <- cbind(X, sfactor[j]*( base_x  ))
    #X <- cbind(X, sfactor[j]*( base_x^2 ))
    #X <- cbind(X, sfactor[j]*( 0.1*(base_x - 1)*(base_x - 0)*(base_x + 1) ))
  }
  #X <- pmin(X,100)
  #X <- pmax(X,-100)
  return(X)
...
# Generate data
generate_data <- function(m,n,gamma0,gamma1,zeta,rho=0){
  
  # Generate a matrix of covariates
  X <- generate_covariates(m = m, n = n)
  # Generate the base null probabilities given X
  linear_pred <- gamma0 + X*gamma1
  base_null_prob <- exp(linear_pred)/(1 + exp(linear_pred))
  #base_null_prob <- pnorm(q = linear_pred)
  H <- matrix(rbinom(m*n, size = 1, prob = as.vector(1 - base_null_prob)), m, n)
