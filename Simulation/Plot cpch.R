...
#        u <- u_n[1]
#        n <- u_n[2]
#        file.name <- paste("Independence/cpch/","mu",mu,"setup",u,n,"xcoef",xcoef,"paral",paral,".RD",sep = "")
#        #load(file = file.name)
#        if (!file.exists(file.name)) {
#          message("Missing: ", file.name)
#          un <- as.integer(paste0(u_n, collapse = ""))
#          failed_ref <-  rbind(failed_ref,c(xcoef,mu,un))
#        }
#      }
#    }
#  }
#}
#
#tally <- table(apply(failed_ref, 1, paste, collapse = ","))
#tally
    ...
    for(u_n in u_n_options){
      ...
      for(paral in paral_options){
        ...
        if (!file.exists(file.name)) {
          ...
          un <- as.integer(paste0(u_n, collapse = ""))
          readjust <- tally[which(names(tally) == paste(c(xcoef,mu,un), collapse = ","))]
          if(length(readjust) == 0) readjust = 0 
          revised_max_paral_options <- max(paral_options) - readjust
          load(file = file.name)
          fdr_dat$value <- fdr_dat$value + cbind.data.frame(reshape2::melt(FDR_list),xcoef,mu,paste(u,n, sep = ""))$value/revised_max_paral_options
          tpr_dat$value <- tpr_dat$value + cbind.data.frame(reshape2::melt(TPR_list),xcoef,mu,paste(u,n, sep = ""))$value/revised_max_paral_options
        }
      }
...
pdf(file = "u_equal_n_Results_full_cpch.pdf",
