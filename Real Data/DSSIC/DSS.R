...
m <- length(gene_names)
p_mat <- matrix(NA, nrow = m, ncol = 2)
rownames(p_mat) <- gene_names
p_mat[,1] <- p_GSE34553[gene_names]
p_mat[,2] <- p_GSE35651[gene_names]

final_covars_GSE34553 <- covars_GSE34553[gene_names]
...
PF_covars <- do.call(cbind, X_list)
...
for(method in methods){
  ...
  if(method == "No-Covar-ParFilter"){
    X_list_no_covar <- list(rep(0,m),rep(0,m))
    R_set <- ParFilter(P = p_mat, u = 2, X_list = X_list_no_covar,
                       K = 2, direction = "negative",
                       q = 0.05, lambda_vec =  rep(lambda,K))
    R_05_list[[method]] <- R_set
  }
  ...
  if(method == "IHW"){
    IHW_covars <- (final_covars_GSE34553 + final_covars_GSE35651[,1] + final_covars_GSE35651[,2])/3
