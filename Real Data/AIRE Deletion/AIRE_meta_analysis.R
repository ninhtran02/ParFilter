...
load("AIREKO.pvalues.Rd")

p.mat <- AIREKO.pvalues
n <- dim(p.mat)[2]
m <- dim(p.mat)[1]
  ...
  for(method in methods){
    for(alpha in alpha_options){
      ...
      if(method == "PF"){
        ...
        obj <- parfilter(p = p.mat, error_targets = c(rep(alpha/10, n),alpha),
...
repAIREKOPlot <- ggplot(repRejection_df, aes(x=alpha,
                                             y = rejections,
                                             group = method,
                                             linetype=method,
                                             shape=method,
...
levels(studyRejectiondf$study) <- c("GSE140683","GSE188870","GSE222285","GSE224247")
studyRejectiondf$rejections <- as.numeric(studyRejectiondf$rejections)
studyRejectiondf$alpha <- as.numeric(studyRejectiondf$alpha)/10
studyAIREKOPlot <- ggplot(studyRejectiondf[studyRejectiondf$u == 'u = 4',], aes(x=alpha,
                                                                                y = rejections,
                                                                                group = method,
                                                                                linetype=method,
                                                                                shape=method,
...
pdf(file = "AIREKOResults44Manual.pdf",  
    width = 10, 
    height = 6) 
ggarrange(repAIREKOPlot,
          studyAIREKOPlot,
          ncol=1, nrow=2, common.legend = TRUE, legend="bottom")
dev.off()


AIREKO_p1_plot <- ggplot(data.frame(GSE140683 = AIREKO.pvalues[,1]), aes(x=GSE140683)) + 
  geom_histogram(aes(y = ..density..)) + geom_vline(xintercept = 0.05, 
                                                    color = "red", size=1)
AIREKO_p2_plot <- ggplot(data.frame(GSE188870 = AIREKO.pvalues[,2]), aes(x=GSE188870)) + 
  geom_histogram(aes(y = ..density..)) + geom_vline(xintercept = 0.05, 
                                                    color = "red", size=1)
AIREKO_p3_plot <- ggplot(data.frame(GSE222285 = AIREKO.pvalues[,3]), aes(x=GSE222285)) + 
  geom_histogram(aes(y = ..density..)) + geom_vline(xintercept = 0.05, 
                                                    color = "red", size=1)
AIREKO_p4_plot <- ggplot(data.frame(GSE224247 = AIREKO.pvalues[,4]), aes(x=GSE224247)) + 
  geom_histogram(aes(y = ..density..)) + geom_vline(xintercept = 0.05, 
                                                    color = "red", size=1)

pdf(file = "AIREKOHistogram.pdf",  
    width = 10,
    height = 3) 
ggarrange(AIREKO_p1_plot,
          AIREKO_p2_plot,
          AIREKO_p3_plot,
          AIREKO_p4_plot,
