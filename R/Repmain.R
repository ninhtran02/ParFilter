...
for(iter in 1:nsim){
  ...
  for(method in methods){
    if(method == "PF"){
      obj <- parfilterp = p, error_targets = c(rep(alpha/5, n),alpha),
    ...
    if(method == "AutoAPF"){
      ...
      #obj <- parfilterp = p, error_targets = rep(alpha, n + 1),
    ...
    if(method == "ad-PF"){
      obj <- parfilterp = p, error_targets = rep(alpha/sum(1/1:m), n + 1),
                       u = u, groups = NULL, group_options = group_options,
                       selections = NULL, u_groups = NULL,
                       adaptive = FALSE, lambda = NULL,
                       auto = FALSE, omega = 0.5, w = NULL,
