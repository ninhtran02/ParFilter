    ...
    for(m in m_vec){
      for(u in u_vec){
        ...
        for(i in 1:m){
          ...
          pc_p <- ParFilter::stouffer_fun(p_vec = p_vec, u = u)
...
pdf(file = "PCdemos_more_n_stouffer",
    width = 9.6, height = 9.6)
PCdemo
dev.off()
```

To reproduce the Figure D.2, run the following R code below:
```
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  install.packages("ggplot2")
}
...
pdf(file = "PCdemos_more_n_fisher",
    width = 9.6, height = 9.6)
PCdemo
dev.off()
```
