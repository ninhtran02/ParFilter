# Reproducing the simulation results in Appendix E.2
...
Rscript --vanilla Repmain_uleqn.R $xcoef $mu $u_n $paral
```
...
3. Create a slurm file called `batch_submission_Repmain_uleqn.slurm` as follows:
```
#!/bin/bash
...
sbatch Repmain_uleqn.slurm $xcoef $mu $u_n $paral
...
Download *Repmain.R*, *DataGene_uleqn.R*, and *Competing Methods.R* from [here](https://github.com/ninhtran02/ParFilter/tree/main/Simulation) and place it in the "Paper Simulations/Covariate-Assisted" folder. 

5. To submit the jobs, run the file *batch\_submission\Repmain.slurm* with the command:
```
sbatch batch_submission_Repmain_uleqn.slurm
```
Generally speaking, you can expect the simulations to finish within half a day or so.

6. The resulting data files saved to the subfolders within *Paper Simulation/Covariate-Assisted/SavedData/uleqn*.
   
7. To produce the plots, download and run *Plot uleqn.R* from [here](https://github.com/ninhtran02/ParFilter/tree/main/Simulation).
