...
create_study_data <- function(x_vec, eta0, xcoef,
  ...
  z_vec <- H_vec*effect_size + Noise # Create z statistics
  
  p_vec <- 1 - pnorm(q = z_vec, mean = 0, sd = 1) # Create p-values
  output <- list(p_vec = p_vec, z_vec = z_vec, x_vec = x_vec, H_vec = H_vec)
  return(output)
}
...
oracle_procedure <- function(meta_study, u, alpha){
  ...
  for(j in 1:n){
    ...
    numerator <- pi_0_vec*dnorm(x = meta_study$z_mat[,j])
    denominator <- numerator + (1-pi_0_vec)*dnorm(x = meta_study$z_mat[,j],
                                              mean = meta_study$effect_size_vec[j],
                                              sd = 1)
