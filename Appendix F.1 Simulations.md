...
```
mkdir -p SavedData/Independence_un/
```

3. Create a slurm file called `Repmain_un.slurm` as follows:
```
#!/bin/bash
#SBATCH --job-name=REP
#SBATCH --nodes=1
...
Rscript --vanilla Repmain_un.R $xcoef $mu $u_n
```
...
3. Create a slurm file called `batch_submission_Repmain_un.slurm` as follows:
```
#!/bin/bash
...
sbatch Repmain_un.slurm $xcoef $mu $u_n
...
Download *Repmain_un.R*, *ParFilter functions.R*, and *Data and Oracle.R* from [here](https://github.com/ninhtran02/ParFilter/tree/main/Simulation) and place it in the "Paper Simulations/Covariate-Assisted" folder. 

5. To submit the jobs, run the file *batch\_submission\_Repmain_un.slurm* with the command:
```
sbatch batch_submission_Repmain_un.slurm
```
Generally speaking, you can expect the simulations to finish within half a day or so.

6. The resulting data files saved to the subfolders within *Paper Simulation/Covariate-Assisted/SavedData/Independence_un*.
   
7. To run produce the plots, download and run *Plot_un.R* from [here](https://github.com/ninhtran02/ParFilter/tree/main/Simulation).
